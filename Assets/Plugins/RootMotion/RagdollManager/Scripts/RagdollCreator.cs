...
namespace RootMotion.Dynamics {
	...
	public abstract class RagdollCreator: MonoBehaviour {
		...
		protected static void ClearTransform(Transform transform) {
			...
			if (IsUtilityNode(transform.gameObject)) return;
		...
		public static void ClearAll_LS(Transform root) {
			if (root == null) return;
			...
			for (int i = animator != null && animator.isHuman? 0: 1; i < transforms.Length; i++) {
				ClearTransform_LS(transforms[i]);
			}
		}
		protected static void ClearTransform_LS(Transform transform) {
			if (transform == null) return;
			if (IsUtilityNode(transform.gameObject))
			{
		...
		public /*protected*/ static void CreateColliderDirectionIsHeight(Transform t, Vector3 startPoint, Vector3 endPoint, ColliderType colliderType, float lengthOverlapProtion, float width)
		{
			Vector3 direction = endPoint - startPoint;
			float height = direction.magnitude * (1f + lengthOverlapProtion);
			Vector3 heightAxis = direction.normalized;
			...
			switch(colliderType) {
				case ColliderType.Capsule:
					...
					capsule.direction = 1; /*DirectionVector3ToInt(1);*/
		...
		public /*protected*/ static void CreateCollider(Transform t, Vector3 startPoint, Vector3 endPoint, ColliderType colliderType, float lengthOverlap, float width) {
			Vector3 direction = endPoint - startPoint;
		...
		public /*protected*/ static void CreateCollider(Transform t, Vector3 startPoint, Vector3 endPoint, ColliderType colliderType, float lengthOverlap, float width, float proportionAspect, Vector3 widthDirection) {
		...
		#region Modified By Hunter (jb) -- 2023年5月10日

		/*protected static void CreateJoint(CreateJointParams p) {*/
		public static void CreateJoint(CreateJointParams p) {
			#endregion
		...
		public static bool IsUtilityNode(Collider collider)
		{
			return collider.isTrigger || IsUtilityNode(collider.gameObject);
		}
		public static bool IsUtilityNode(GameObject gameObject)
		{
			return gameObject.layer == LayerMask.NameToLayer("Combat_hitBox") || gameObject.layer == LayerMask.NameToLayer("Combat_hurtBox");
		}
