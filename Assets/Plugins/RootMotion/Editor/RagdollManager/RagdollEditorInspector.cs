...
namespace RootMotion.Dynamics {
	...
	public class RagdollEditorInspector : Editor {
		...
		public override void OnInspectorGUI() {
			...
			var puppet = script.GetComponent<PuppetMaster>();
			if (puppet != null) return;
			if (GUILayout.Button("生成布娃娃对象"))
			{
				if (PrefabUtility.IsPartOfPrefabInstance(script.gameObject))
					PrefabUtility.UnpackPrefabInstance(
						PrefabUtility.GetNearestPrefabInstanceRoot(script.gameObject), PrefabUnpackMode.Completely,
						InteractionMode.UserAction);
				puppet = script.gameObject.AddComponent<PuppetMaster>();
				var ragdoll = puppet.SetUpTo(script.transform, 6, 22);
				if (ragdoll != null)
				{
					var ragdollRoot = ragdoll.transform.parent;
					ragdollRoot.gameObject.AddComponent<RagdollBinder>();
					ragdollRoot.name = ragdollRoot.name.Replace("(Clone) Root", "");
					Selection.activeObject = ragdollRoot.gameObject;
					EditorSceneManager.MarkSceneDirty(EditorSceneManager.GetActiveScene());
				}
				else
				{
					Debug.LogError("生成布娃娃对象失败！");
				}
			}
		}
