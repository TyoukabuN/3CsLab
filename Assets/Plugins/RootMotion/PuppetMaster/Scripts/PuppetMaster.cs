...
namespace RootMotion.Dynamics
{
    ...
    {
        #region Modified By Hunter (jb) -- 2022年7月8日
        ...
        public void ApplyMasterSetting(MasterSetting setting)
        {
            this.mappingWeight = setting.mappingWeight;
            this.pinWeight = setting.pinWeight;
            this.muscleWeight = setting.muscleWeight;
            this.muscleSpring = setting.muscleSpring;
            this.muscleDamper = setting.muscleDamper;
            this.angularPinning = setting.angularPinning;
            this.updateJointAnchors = setting.updateJointAnchors;
            this.angularLimits = setting.angularLimits;
            this.internalCollisions = setting.internalCollisions;
        }
        ...
        #region mod by hunter

        public delegate bool MapJudge(bool shouldMap);

        public MapJudge OverrideJudgeMap;
        ...
        public UpdateDelegate OnPostMuscleUpdate;
        ...
        /// Gets the Animator on the target.
        /// </summary>
        public AnimancerComponent targetAnimancer
        {
            get
            {
                // Protect from the Animator being replaced (UMA)
                if (_targetAnimancer == null) _targetAnimancer = targetRoot.GetComponentInChildren<AnimancerComponent>();
                if (_targetAnimancer == null && targetRoot.parent != null) _targetAnimancer = targetRoot.parent.GetComponentInChildren<AnimancerComponent>();
                return _targetAnimancer;
            }
            set
            {
                _targetAnimancer = value;
            }
        }
        private AnimancerComponent _targetAnimancer;
        ...
        public bool FixedFrame => fixedFrame;
        ...
        public bool ForceInterpolate { get; set; }
        ...
        public bool IsPuppetValid { get; private set; }
...
*/
        ...
        private void Initiate()
        {
            ...
            if (targetRoot != null && targetAnimancer == null)
            {
                if (targetAnimancer == null) targetAnimancer = targetRoot.GetComponentInChildren<AnimancerComponent>();
            }

            // Validation
            if (!IsValid(true, this))
            {
                IsPuppetValid = false;
                return;
            }
            ...
            for (int i = 0; i < muscles.Length; i++)
            {
                ...
                muscles[i].Initiate(muscles, this);
        ...
        public void OnPreSimulate(float deltaTime)
        {
            ...
            FixTargetTransforms();
            if (targetAnimancer != null)
            {
                if(targetAnimancer.IsPlaying()) targetAnimancer.Playable.PauseGraph();
                
                targetAnimancer.Evaluate(deltaTime);
            }
        ...
        public void OnPostSimulate()
        {
            ...
            if (initiated)
            {
                ...
                if (!isFrozen)
                {
                    ...
                    //puppetMaster插件蛋疼的两个地方:
                    //1. 核心功能和便捷调用混在一起, 想精细控制各种坑
                    //2. weight用乘数蛋疼
                    var shouldMap = mW > 0f;
                    if (OverrideJudgeMap != null)
                    {
                        shouldMap = OverrideJudgeMap.Invoke(shouldMap);
                    }
                    //if (mW > 0f)
                    if (shouldMap)
                        #endregion
        ...
        protected virtual void FixedUpdate()
        {
            ...
            if (updateMode == UpdateMode.FixedUpdate)
            {
                ...
                if (targetAnimator.enabled && (targetAnimancer != null ||(!targetAnimancer.enabled && animatorDisabled)))
                {
                    if(targetAnimancer.IsPlaying()) targetAnimancer.Playable.PauseGraph();
                    targetAnimancer.Evaluate(Time.fixedDeltaTime);
                    animatorDisabled = true;
                }
                else if (targetAnimator.enabled ||  (!targetAnimator.enabled && animatorDisabled))
                {
        ...
        protected virtual void Update()
        {
            ...
            if (animatorDisabled)
            {
                targetAnimancer.Playable.UnpauseGraph();
        ...
        public bool manualLateUpdate { get; set; }

        public void LateUpdateExternal()
        {
            manualLateUpdate = true;
            LateUpdateInternal();
        }
        ...
        protected virtual void OnLateUpdate()
        {
            ...
            switch (updateMode)
            {
                case UpdateMode.FixedUpdate:
                    ...
                    if (!fixedFrame && (!interpolated && !ForceInterpolate)) return;
                    break;
