...
namespace RootMotion.Dynamics {
	...
	public partial class PuppetMaster: MonoBehaviour {
		...
		/// </summary>
		public bool IsValid(bool log, PuppetMaster puppetMaster) {
			if (muscles == null) {
				if (log) Debug.LogError("PuppetMaster Muscles is null.", puppetMaster);
				validationError = "PuppetMaster Muscles is null.";
				return false;
			}
			
			if (muscles.Length == 0) {
				if (log) Debug.LogError("PuppetMaster has no muscles.", puppetMaster);
				validationError = "PuppetMaster has no muscles.";
				return false;
			}
			
			for (int i = 0; i < muscles.Length; i++) {
				if (muscles[i] == null) {
					if (log) Debug.LogError("Muscle is null, PuppetMaster muscle setup is invalid.", puppetMaster);
					validationError = "Muscle is null, PuppetMaster muscle setup is invalid.";
					return false;
				}

				if (!muscles[i].IsValid(log, true, this))
				{
					validationError = $"Muscle {i} is not valid, check log";
					return false;
				}
			}

			if (targetRoot == null) {
				if (log) Debug.LogError("'Target Root' of PuppetMaster is null.",puppetMaster);
				return false;
			}
