...
namespace RootMotion.Dynamics {
	...
	public partial class PuppetMaster: MonoBehaviour {
		...
		public static PuppetMaster SetUp_LS(Transform target, int characterControllerLayer, int ragdollLayer, LayerMask removeColliderInLayer) {
			Transform ragdoll = ((GameObject)GameObject.Instantiate(target.gameObject, target.transform.parent, true)).transform;

			PuppetMaster puppetMaster = ragdoll.gameObject.AddComponent<PuppetMaster>();
			puppetMaster.SetUpTo_LS(target, characterControllerLayer, ragdollLayer,removeColliderInLayer);

			// Clean up the target from all the ragdoll components
			// if(!skipSourceComponentRemove)
			// 	RemoveRagdollComponents(target, characterControllerLayer);
		...
		/// </summary>
		public GameObject SetUpTo(Transform setUpTo, int characterControllerLayer, int ragdollLayer) {
			if (setUpTo == null) {
			...
			*/
			...
			return setUpTo.gameObject;
		}

		#region Modified By Hunter (jb) -- 2023年5月5日

		public GameObject SetUpTo_LS(Transform setUpTo, int characterControllerLayer, int ragdollLayer, LayerMask removeLayerFromRagdollRig) {
			RemoveUnnecessaryBones_LS(removeLayerFromRagdollRig);
			/*
			Animator[] animators = GetComponentsInChildren<Animator>();
			for (int i = 0; i < animators.Length; i++) {
				DestroyImmediate(animators[i]);
		...
		public static void RemoveRagdollComponents(Transform target, int characterControllerLayer) {
			...
			for (int i = 0; i < rigidbodies.Length; i++) {
				if (rigidbodies[i].gameObject != target.gameObject) {
					...
					if (collider != null) {
						if (!IsClothCollider(collider, cloths) && !RagdollCreator.IsUtilityNode(collider)) DestroyImmediate(collider);
			...
			for (int i = 0; i < colliders.Length; i++) {
				if (colliders[i].transform != target && !IsClothCollider(colliders[i], cloths) && !RagdollCreator.IsUtilityNode(colliders[i])) 
				{
		...
		private void RemoveUnnecessaryBones_LS(LayerMask removeLayers)
		{
			Transform[] children = GetComponentsInChildren<Transform>();
			for (int i = 1; i < children.Length; i++) {
				bool keep = false;
				...
				if (children[i].GetComponent<Collider>() != null && children[i].GetComponent<Rigidbody>() == null) keep = true; // Compound collider
				if (children[i].GetComponent<CharacterController>() != null) keep = false; // Character controller
				if (((1 << children[i].gameObject.layer) & removeLayers) != 0) keep = false;
