...
namespace RootMotion.Dynamics
{
    ...
    public class Muscle
    {
        ...
        public class Props
        {
            ...
            #region Modified By Hunter (jb) -- 2023年4月21日

            public bool isSpine => group is Group.Head or Group.Spine or Group.Hips;
        ...
        #region Modified By Hunter (jb) -- 2023年4月16日

        public bool KinematicFollowTarget { get;private set; }
        ...
        */
        ...
        public bool IsValid(bool b, bool log, UnityEngine.Object puppetMaster)
        {
            if (joint == null)
            {
                if (log) Debug.LogError("Muscle joint is null",puppetMaster);
                return false;
            }

            if (target == null)
            {
                if (log) Debug.LogError("Muscle " + joint.name + " target is null, please remove the muscle from PuppetMaster or disable PuppetMaster before destroying a muscle's target.", puppetMaster); return false;
            }

            if (props == null)
            {
                if (log) Debug.LogError("Muscle " + joint.name + " props is null",puppetMaster);
            }
        ...
        public virtual void Initiate(Muscle[] colleagues, PuppetMaster puppetMaster)
        {
            initiated = false;
            if (!IsValid(true,true, puppetMaster)) return;
       ...
       // Moves targetl localPosition back to default (used for reconnecting limb in Dead state when animation is not overriding target localPosition)
        ...
        public void SetKinematic(bool to)
        {
            ...
            if (!to) KinematicFollowTarget = false;
            rigidbody.isKinematic = to;
            if (additionalPin != null) additionalRigidbody.isKinematic = to;
        }

        public void SetKinematicAndFlowTarget()
        {
            SetKinematic(true);
            KinematicFollowTarget = true;
        }
        ...
        #region Modified By Hunter (jb) -- 2022年7月6日

        public delegate void MapDelegate(Muscle muscle, Vector3 toPos, Quaternion toRot);

        public delegate float WeightDelegate(Muscle muscle, float defaultWeight);
        public MapDelegate OverrideMap { get; set; }
        public WeightDelegate OverrideMappingWeight { get; set; }
        public WeightDelegate OverridePinWeight { get; set; }
        ...
        public void Map(float mappingWeightMaster)
        {
            ...
            if (OverrideMappingWeight != null)
            {
                w = OverrideMappingWeight.Invoke(this, w);
            }
            // float w = props.mappingWeight * mappingWeightMaster * state.mappingWeightMlp; 
        ...
        public Vector3 LastMappedPosition => lastMappedPosition;
        ...
        public Quaternion LastMappedRotation => lastMappedRotation;
        ...
        private void Pin(float pinWeightMaster, float pinPow, float pinDistanceFalloff, bool angularPinning, float deltaTime)
        {
            ...
            if (OverridePinWeight != null)
            {
                w = OverridePinWeight(this, w);
            }
            if (w <= 0f) return;
